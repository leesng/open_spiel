...
std::vector<std::pair<int, int>> game::get_current_boards_edges(std::optional<std::vector<std::pair<int,std::vector<uint64_t>>>>& out_all_boards) const
{
    const state& s = get_current_state();
    auto [all_boards, boards_edges] = s.get_boards_and_edges();
	if (out_all_boards.has_value()) {
		out_all_boards = all_boards;
	}
    return boards_edges;
}
...
bool game::big_round_over(bool &can_pass) const {return get_current_state().big_round_over(can_pass);}
...
{
    std::ostringstream oss;
    constexpr static std::array<const char*, 9> ordered_keys = {
